 ...
 * });
 */
exports.writeImage = (imagePath, drives, options, onProgress) => {
  return drivelist.listAsync().then((devices) => {
    const selectedDrives = devices.filter((drive) => {
      return drives.includes(drive.device)
    })

    if (!selectedDrives || !selectedDrives.length) {
