    ...
    ipc.server.on('start', () => {
      ...
      permissions.elevateCommand(argv, {
        ...
        environment: {
          ...
          OPTION_IMAGE: image,
          OPTION_DEVICE: drives.map( d => d.device ).join(),
...
exports.flash = (image, drives) => {
  ...
  const writerPromises = _.reduce(drives, (driveObject, drive) => {
    driveObject[drive.raw] = new Bluebird((resolve, reject) => {
      resolvers[drive.raw] = resolve
      rejecters[drive.raw] = reject
    })
    return driveObject
  }, {})

  exports.performWrite(image, drives, (currentState) => {

    states[currentState.raw] = currentState
    ...
    console.log('then:what', argv)
    return flashState.unsetFlashingFlag()
  }).then(() => {
  ...
  return Bluebird.resolve(writerPromises)
}
