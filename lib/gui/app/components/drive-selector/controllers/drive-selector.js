...
module.exports = function (
  ...
  const shouldChangeDriveSelectionState = (drive) => {
    ...
    if (constraints.isDriveSizeLarge(drive)) {
      return confirmationWarningModal(messages.warning.largeDriveSize({
    ...
    if (!constraints.isDriveSizeRecommended(drive, selectionState.getImage())) {
      return confirmationWarningModal(messages.warning.unrecommendedDriveSize({
        drive,
  ...
  /**
   * @summary Is the selected drive labelled with a warning or error?
   * @function
   * @public
   *
   * @example
   * if (DriveSelectorController.isSelectedDriveDangerous()) {
   *   console.log('Selected drive is dangerous')
   * }
   */
  this.isSelectedDriveDangerous = () => {
    return Boolean(this.getDriveStatuses(selectionState.getDrive()).length)
  }
}
