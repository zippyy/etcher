...
module.exports = function (
  ...
  const shouldChangeDriveSelectionState = (drive) => {
    ...
    const isLargeDrive = constraints.isDriveSizeLarge(drive)
    const isRecommendedDrive = constraints.isDriveSizeRecommended(drive, selectionState.getImage())

    if (isLargeDrive || !isRecommendedDrive) {
      const messageTuples = [
        [
          isLargeDrive,
          messages.warning.largeDriveSize({
      ...
      const concatenatedMessage = _.reduce(messageTuples, (accumulatedMessage, messageTuple) => {
          if (messageTuple[0]) {
      ...
      return confirmationWarningModal(concatenatedMessage)
    }
