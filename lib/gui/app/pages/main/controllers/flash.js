...
module.exports = function (
  ...
  const displayTailoredWarning = (drive, image) => {
    const messageKeys = {
      largeDriveSize: constraints.isDriveSizeLarge(drive),
      unrecommendedDriveSize: !constraints.isDriveSizeRecommended(drive, image)
    }
    const messageArgs = {
      largeDriveSize: {
    ...
    const messageKey = _.findKey(messageKeys, _.identity)
    const message = messages.warning[messageKey](messageArgs[messageKey])

    if (!message) {
      return errors.createError({
        title: `No such warning message: ${messageKey}`
      })
    }
