...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    ...
    case ACTIONS.SET_FLASH_STATE: {
      ...
      const requiredFields = [ 'speed', 'humanSpeed', 'eta', 'type', 'percentage' ]
      const missingFields = _.reject(requiredFields, _.partial(_.has, action.data))

      if (missingFields.length) {
        throw errors.createError({
          title: `Missing state fields: ${missingFields.join(', ')}`
        })
      }
